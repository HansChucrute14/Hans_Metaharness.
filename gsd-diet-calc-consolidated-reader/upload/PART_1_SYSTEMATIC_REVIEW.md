# UNITED PROJECT DOSSIER — Hans-GSD-Raw-Calculator (gsd-diet-calc v10.4.0)

## How to read this dossier (3 sequential parts)

This dossier unites six source documents into three consecutive, sequential parts that follow the natural lifecycle of the review-and-remediation effort for `github.com/HansChucrute14/Hans-GSD-Raw-Calculator`:

| Part | Title | Role in the sequence | Source documents consolidated |
|---|---|---|---|
| **Part 1** *(this file)* | **Systematic Adversarial Review** | **The findings / evidence layer** — what is wrong, verified against source. | `SYSTEMATIC_REVIEW_REPORT (1).md` |
| **Part 2** | **Remediation Plan & Roadmap** | **The remediation layer** — how to fix it, in three progressive stages: the detailed plan → the synthesized executive roadmap → the amendment that corrects/extends it. | `REMEDIATION_PLAN.md` + `EXECUTIVE_REMEDIATION_ROADMAP.md` + `ROADMAP_G1_AMENDMENT.md` |
| **Part 3** | **Executive Map & Direct Repo Verification** | **The synthesis layer** — a single executive map that ties everything together, then a hands-on re-verification against the live repository that confirms the findings and corrects two of them. | `MAPA_DO_PROJETO.md` + `MAPA_DO_PROJETO_2.0.md` |

**Reading order:** Part 1 (evidence) → Part 2 (plan) → Part 3 (synthesis & verification). Each part is self-contained but builds on the previous one. No repository file was modified by any part — the entire dossier is a planning/review artifact. All numeric safety values flagged "verify-first" require AAFCO/NRC/FEDIAF primary-source confirmation + veterinary review before implementation, and no diet from this system may be fed to an animal until the P0 items are fixed and a board-certified veterinary nutritionist signs off.

---

# PART 1 — SYSTEMATIC ADVERSARIAL REVIEW
## Consolidated Master Report (all five reviewer streams unified)
### Hans-GSD-Raw-Calculator (gsd-diet-calc v10.4.0)
...
**End of Part 1.** Continue to **Part 2 — Remediation Plan & Roadmap** for the actionable fix sequence built on these findings.
